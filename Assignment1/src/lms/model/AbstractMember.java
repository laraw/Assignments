...
public abstract class AbstractMember implements Member {
	...
	/** The borrow holding method accepts any type of holding as a parameter. If a holding has been previously been
	 loaned to a member, they are no longer able to borrow it. Additionally, a holding can only be borrowed by a member
	 if the member has sufficient credit. **/
	
	public void borrowHolding(Holding holding) throws LMSException {	
			
		// First, check that the holding has not been borrowed before, if it has, throw an exception
		...
		// Check if the member has sufficient credit. If they do not, throw an exception. 
		...
		/** Once a holding is borrowed, it should be set to on loan, and the members current list of holdings
			are updated. At the time of the transaction, the member will pay the standard loan fee **/
	...
	// A placeholder for the return holding method, the implementation will depend on the type of member. 
	
	public abstract void returnHolding(Holding holding) throws LMSException;
	
	// Deducts a late fee from the members credit when a holding is returned by the member. 
